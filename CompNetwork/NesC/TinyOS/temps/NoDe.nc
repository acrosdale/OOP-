...
implementation{
	...
	command void Transport.print (uint8_t *buff, uint16_t bufflen){
		...
		for(i = 0; i < bufflen; i++){
			...
			dbg(TRANSPORT_CHANNEL, "Reading Data: %hhu %hhu %hhu %hhu %hhu %hhu\n",buff[i],buff[i+1],buff[i+2],buff[i+3],buff[i+4],buff[i+5] );
			i+=5;
		}
	...
	command uint16_t Transport.read(socket_t *fd, uint8_t *buff, uint16_t bufflen){
		...
		if(fd->state == EST){
			...
			if(fd->last_recieve < fd->last_read){ //normal case
				...
				if(bufflen <  (fd->last_recieve - fd->last_read)  ){ //TAKE ALL OF IT
				//dbg(TRANSPORT_CHANNEL, "DAT");
					len = bufflen;
				}
				else{
				//dbg(TRANSPORT_CHANNEL, "DAT2");
					len = fd->last_recieve - fd->last_read;

				}
			}


			if(fd->last_recieve > fd->last_read){ //wrap around
		...
		//dbg(TRANSPORT_CHANNEL, "				DATA Slen: %hhu\n",len);
		...
		//FD_src->last_read = ;
		//fd->next_expected  = fd->last_read;
		//fd->last_recieve  = fd->last_read;
		//fd->next_expected = fd->last_read+1;

		/*dbg(TRANSPORT_CHANNEL, "DATA RECIEVED FROM PORT: %d\n",fd->Src_port );

		for(i = 0; i < BUFFER_SIZE ; i++){

			dbg(TRANSPORT_CHANNEL,"DATA %hhu %hhu %hhu\n", buff[i],buff[i+1],buff[i+2]);
			i+=2;
    ...
    command uint16_t Transport.write(socket_t* fd, uint8_t *buff, uint16_t bufflen){
		...
		if(fd->state == EST){
			...
			if(fd->last_write > fd->last_ack){
				...
				if(bufflen < (BUFFER_SIZE -(fd->last_write - fd->last_ack) ) ){ //TAKE ALL OF IT
					...
					len = (BUFFER_SIZE -(fd->last_write - fd->last_ack)); //take all the remaining buffer
				}
				...
				if(bufflen < (BUFFER_SIZE -(fd->last_send - fd->last_ack) ) ){ //TAKE ALL OF IT
					len = bufflen;
				}

				else{ // TAKE A PORTION
					len = (BUFFER_SIZE -(fd->last_send - fd->last_ack)); //take all the remaining buffer
				}
		...
		//dbg(TRANSPORT_CHANNEL,"DATA SENDING??:%hhu\n", fd->S_buff[fd->last_write % BUFFER_SIZE]);
		//dbg(TRANSPORT_CHANNEL,"DATA SENDING???:%hhu\n", fd->S_buff[(fd->last_write) % BUFFER_SIZE]);

		//the amount of data left to be transmitted
		WORK_LEFT -= len;
    ...
    command error_t Transport.receive(pack* package){
		...
		if(tcp->Flag == (SYN_ACK) && FD_src->state == SYN_SEND){
			...
			//dbg(TRANSPORT_CHANNEL, "POINTERS CLIENT  %hu %hu %hu\n",tcp->Acknowledge, tcp->SeqNum,tcp->SeqNum);
		...
		if(tcp->Flag == DAT && FD_src->state == EST){
			...
			//@%@@ Find a better to do this because because moving timer back and forth is bad
			...
			//FD_src->last_recieve   = tcp->length;
			//FD_src->last_read      = tcp->length;

			//dbg(TRANSPORT_CHANNEL, "NEXT?? %hhu\n",FD_src->next_expected);
			//dbg(TRANSPORT_CHANNEL, "LAST? R %hhu\n",FD_src->last_recieve);
			//dbg(TRANSPORT_CHANNEL, "LAST? RE%hhu\n",FD_src->last_read);
		...
		if(tcp->Flag == FIN  && FD_src->state == EST){
			...
			dbg(TRANSPORT_CHANNEL, "FIN to close 1\n");
			TCP = (Transfer*) MSG.payload;
		...
		if(tcp->Flag == ACK && FD_src->state == FIN_WAIT){//server state closing
			...
			dbg(TRANSPORT_CHANNEL, "ACK to close 1...\n");
		...
		if(tcp->Flag ==  FIN && FD_src->state == FIN_WAIT_2){ //client state
			...
			dbg(TRANSPORT_CHANNEL, "FIN_2 to close\n");
		...
		if(tcp->Flag == ACK && FD_src->state == CLOSED_WAIT){  //server state closing
			...
			dbg(TRANSPORT_CHANNEL, "ACK to closed 2 \n");
			FD_src->state = CLOSED;
